...
function Bar({ mastered, learning, total }: { mastered: number; learning: number; total: number }) {
  ...
  return (
    <span className="readout flex h-3 flex-1 overflow-hidden p-px shadow-none">
      <span className="h-full bg-readout" style={{ width: `${m}%` }} />
      <span className="h-full bg-readout/35" style={{ width: `${l}%` }} />
    </span>
  )
}
...
function ItemRow({ item, note, noteClass }: { item: StudyItem; note: string; noteClass: string }) {
  return (
    <div className="lens flex items-baseline justify-between gap-4 px-3 py-2">
      <div className="min-w-0">
        <div className="truncate font-mono text-sm font-semibold text-legend">
          {itemLabel(item)}
          {item.kind === 'flag' && (
            <span className="placard ml-2 text-[8px]">flag</span>
          )}
        </div>
        <div className="truncate font-sans text-xs text-legend-dim">{itemDesc(item)}</div>
      </div>
      <span className={`shrink-0 font-sans text-xs font-semibold ${noteClass}`}>{note}</span>
    </div>
  )
}
...
export function LearnOverview() {
  ...
  if (!anyProgress) {
    return (
      <p className="rounded-sm border border-dashed border-console-edge px-4 py-6 text-center font-sans text-[13px] text-legend-dim">
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="mb-2 flex items-baseline justify-between px-1">
          <span className="font-sans text-sm text-legend-dim">
            <span className="font-mono font-semibold text-readout">{mastered.length}</span>
          ...
          <button
            ...
            className={`font-sans text-xs transition-colors ${
              resetArmed ? 'font-bold text-stop' : 'text-legend-dim underline hover:text-legend'
            }`}
          >
      ...
      <div className="flex flex-col gap-2">
        {byCategory.map((c) => (
          <div key={c.id} className="flex items-center gap-3">
            <span className="w-28 shrink-0 font-mono text-xs text-legend">{categoryLabel(c.id)}</span>
            <Bar mastered={c.mastered} learning={c.learning} total={c.total} />
            <span className="w-16 text-right font-sans text-xs text-legend-dim tabular-nums">
              {c.mastered}/{c.total}
            </span>
          </div>
        ))}
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {recentlyMastered.length > 0 && (
          <div>
            <h3 className="placard mb-2 text-[9px]">recently mastered</h3>
            <div className="flex flex-col gap-2">
              {recentlyMastered.map(([i]) => (
                <ItemRow key={studyKey(i)} item={i} note="✓" noteClass="text-run" />
              ))}
            </div>
          </div>
        )}
        {needsWork.length > 0 && (
          <div>
            <h3 className="placard mb-2 text-[9px]">needs work</h3>
            <div className="flex flex-col gap-2">
              {needsWork.map(([i, r]) => (
                <ItemRow
                  key={studyKey(i)}
