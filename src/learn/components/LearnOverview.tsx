...
function Bar({ mastered, learning, total }: { mastered: number; learning: number; total: number }) {
  ...
  return (
    <span className="bevel-down flex h-2.5 flex-1 overflow-hidden p-px">
      <span className="h-full bg-teal" style={{ width: `${m}%` }} />
      <span className="h-full bg-teal/35" style={{ width: `${l}%` }} />
    </span>
  )
}
...
function ItemRow({ item, note, noteClass }: { item: StudyItem; note: string; noteClass: string }) {
  return (
    <div className="bevel-down flex items-baseline justify-between gap-4 px-3 py-2">
      <div className="min-w-0">
        <div className="truncate font-mono text-sm text-ink">
          {itemLabel(item)}
          {item.kind === 'flag' && (
            <span className="engraved ml-2 px-1 text-[8px]">flag</span>
          )}
        </div>
        <div className="truncate font-sans text-xs text-ink-faint">{itemDesc(item)}</div>
      </div>
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="mb-2 flex items-baseline justify-between px-1">
          <span className="font-sans text-sm text-ink-soft">
            <span className="font-mono font-semibold text-teal-deep">{mastered.length}</span>
            <span className="text-ink-faint"> / {items.length}</span> mastered in your selection
            ...
            {learning.length > 0 && (
              <span className="text-ink-faint"> · {learning.length} in progress</span>
            )}
          </span>
          <button
            type="button"
            ...
            className={`font-sans text-xs transition-colors ${
              resetArmed ? 'font-semibold text-[#b8402c]' : 'text-ink-faint hover:text-ink-soft'
            }`}
          >
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {recentlyMastered.length > 0 && (
          <div>
            <h3 className="engraved mb-2 text-[9px]">recently mastered</h3>
            <div className="flex flex-col gap-2">
              {recentlyMastered.map(([i]) => (
                <ItemRow key={studyKey(i)} item={i} note="✓" noteClass="text-teal-deep" />
              ))}
            </div>
          </div>
        )}
        {needsWork.length > 0 && (
          <div>
            <h3 className="engraved mb-2 text-[9px]">needs work</h3>
            <div className="flex flex-col gap-2">
              {needsWork.map(([i, r]) => (
                <ItemRow
                  key={studyKey(i)}
