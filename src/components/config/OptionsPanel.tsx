...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-2">
      <span className="text-[13px] font-semibold text-nylon">{label}</span>
      {children}
    </div>
  )
}
      ...
      <div className="flex flex-col gap-4">
        <Row label="prompt">
          <select
            ...
            className="border-2 border-nylon bg-white px-2.5 py-1.5 font-mono text-[13px] text-nylon outline-none"
          >
