  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between px-1">
        <h3 className="text-sm text-legend">
          progress · last {recent.length} tests
        </h3>
        <span className="text-xs text-legend-dim">
          best <b className="font-mono text-readout">{fmtInt(best)}</b> · latest{' '}
          <b className="font-mono text-legend">{fmtInt(last.wpm)}</b> wpm
        </span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        ...
        />
        {pts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="2.2" fill="var(--w-readout)" />
        ))}
      </svg>
    </div>
  )
