...
export function WpmChart({ bins }: { bins: SecondBin[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {bins
          ...
          .map((b) => (
            ...
            >
              <circle cx={x(b.sec)} cy={y(b.raw)} r="3.5" fill="var(--w-key-yellow)" />
            </motion.g>
          ))}
      ...
      {hovered && (
        <div
          className="tempo pointer-events-none absolute -translate-x-1/2 px-3 py-1.5 font-mono text-xs"
          style={{
        ...
        >
          ...
          {hovered.errors > 0 && (
            <span className="text-key-yellow"> · {hovered.errors} err</span>
          )}
        </div>
      )}

      <div className="silk mt-2 flex justify-center gap-5 text-[8px]">
        <span className="inline-flex items-center gap-1.5">
          <span className="h-[2px] w-4 bg-tempo" /> wpm
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-px w-4 bg-silk-dim" /> raw
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2 rounded-full bg-key-yellow" /> errors
        </span>
      </div>
    </div>
  )
