...
export function WpmChart({ bins }: { bins: SecondBin[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
          ...
          return (
            <g key={i}>
              <line
                ...
                y1={gy}
                y2={gy}
                stroke="color-mix(in srgb, var(--w-lcd-dim) 45%, transparent)"
                strokeWidth="1"
              />
              <text
                x={PAD.left - 8}
        ...
        {bins
          ...
          .map((b) => (
            ...
            >
              <circle cx={x(b.sec)} cy={y(b.raw)} r="3.5" fill="var(--w-lcd-err)" />
            </motion.g>
          ))}
      ...
      {hovered && (
        <div
          className="bevel-up pointer-events-none absolute -translate-x-1/2 px-3 py-1.5 font-sans text-xs"
          style={{
            left: `${(x(hovered.sec) / W) * 100}%`,
            top: 0,
          }}
        >
          <span className="text-ink-soft">{hovered.sec}s · </span>
          <span className="font-semibold text-teal-deep">{fmtInt(hovered.smooth)} wpm</span>
          {hovered.errors > 0 && (
            <span className="text-[#b8402c]"> · {hovered.errors} err</span>
          )}
        </div>
      )}

      <div className="mt-1 flex justify-center gap-5 font-sans text-[11px] text-lcd-dim">
        <span className="inline-flex items-center gap-1.5">
          <span className="h-0.5 w-4 bg-lcd-fg" /> wpm
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-0.5 w-4 bg-lcd-dim" /> raw
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2 rounded-full bg-lcd-err" /> errors
        </span>
      </div>
    </div>
  )
