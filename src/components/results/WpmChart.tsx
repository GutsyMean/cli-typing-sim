...
export function WpmChart({ bins }: { bins: SecondBin[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {bins
          ...
          .map((b) => (
            ...
            >
              <circle cx={x(b.sec)} cy={y(b.raw)} r="3.5" fill="var(--w-spec-blue)" />
            </motion.g>
          ))}
      ...
      {hovered && (
        <div
          className="pointer-events-none absolute -translate-x-1/2 border border-ink/40 bg-paper-hi px-3 py-1.5 font-sans text-xs shadow-lg"
          style={{
        ...
        >
          ...
          <span className="font-semibold text-safety">{fmtInt(hovered.smooth)} wpm</span>
          {hovered.errors > 0 && (
            <span className="text-spec-blue"> · {hovered.errors} err</span>
          )}
        </div>
      )}

      <div className="mt-1 flex justify-center gap-5 font-sans text-[11px] text-ink-soft">
        <span className="inline-flex items-center gap-1.5">
          <span className="h-0.5 w-4 bg-safety" /> wpm
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-0.5 w-4 bg-ink-faint" /> raw
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2 rounded-full bg-spec-blue" /> errors
        </span>
      </div>
    </div>
  )
