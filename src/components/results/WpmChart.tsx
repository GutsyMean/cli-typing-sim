...
export function WpmChart({ bins }: { bins: SecondBin[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {bins
          ...
          .map((b) => (
            ...
            >
              <circle cx={x(b.sec)} cy={y(b.raw)} r="3.5" fill="var(--w-closed)" />
            </motion.g>
          ))}
      ...
      {hovered && (
        <div
          className="sign-band pointer-events-none absolute -translate-x-1/2 px-3 py-1.5 text-xs font-bold"
          style={{
            left: `${(x(hovered.sec) / W) * 100}%`,
            top: 0,
          }}
        >
          <span>{hovered.sec}s · </span>
          <span>{fmtInt(hovered.smooth)} wpm</span>
          {hovered.errors > 0 && (
            <span className="text-closed"> · {hovered.errors} err</span>
          )}
        </div>
      )}
      ...
      <div className="mt-1 flex justify-center gap-5 text-[11px] font-bold text-white">
        ...
        <span className="inline-flex items-center gap-1.5">
          <span className="h-[2px] w-4 bg-white/50" /> raw
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="size-2 rounded-full bg-closed" /> errors
        </span>
      </div>
    </div>
  )
