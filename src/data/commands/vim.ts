...
export const vim: CommandEntry[] = [
  ...
  { text: "vim -u NONE -N -c 'set nowrap' huge.log", category: 'vim', difficulty: 3, desc: 'open a huge file fast, no config, no wrapping' },
  { text: "vim -es -c '%s/foo/bar/g' -c 'wq' batch.txt", category: 'vim', difficulty: 3, desc: 'headless search and replace from a script' },
  { text: ':windo diffthis', category: 'vim', difficulty: 3, desc: 'diff all the windows you already have open' },
  { text: ":cexpr system('grep -rn TODO src') | copen", category: 'vim', difficulty: 3, desc: 'load grep hits into the quickfix list' },
]
